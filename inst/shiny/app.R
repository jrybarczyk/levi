...
levi_shiny = function(expression, fileType, networkEdges,
    ...
    if (length(naTotal) > 0) {
        showNotification(paste0("There are ",nrow(naTotal),
            " nodes without expression value, see log in path: ",
            file.path(tempdir(),titleChart, "levi.log")), type = "warning",
    ...
    coordPiso <- SigCoordPiso(coord= coord,  backValue = backValue,
        ...
        alphaValue = alphaValue, betaValue=betaValue, numberCoord=numberCoord)
    ...
    matrix_resultado <- matrix_entrada(coordPiso= coordPiso,
        SignalOut= SignalOut, coord= coord, backValue= backValue,
    ...
    sidebarLayout(
        ...
        sidebarPanel(
            ...
            tabsetPanel(id = "tabset_id", selected = "t1",
                ...
                c("Medusa (DAT)" = "dat", "RedeR (DYN)" = "dyn","Pajek (NET)" =
                "net", "STRING / STITCH" = "stg")),
                br(),
                uiOutput("out2"),
                fileInput("file","Upload the expression file:"),
                checkboxInput("log", "Expression values in log scale", FALSE),
                radioButtons("fields", "Selected fields:",
                c("Two Samples" = "twofields",
                "One Sample" = "onefield"), selected = "twofields",
                inline = TRUE),
                uiOutput("out1"),
                checkboxInput("contour", "Chart with contour", TRUE),
                tags$hr(),
                ...
                sliderInput("contrast", "Contrast:", min = 0, max = 100,
                value = 50), sliderInput("size", "Resolution:", min = 1,
                max = 100, value = 50), sliderInput("smooth", "Smoothing:",
                min = 0, max = 100, value = 50),sliderInput("zoom", "Zoom:",
...
server <- function(input, output, session) {
    ...
    output$out1 <- renderUI({
        dynUi <- tabPanel("", value = "t1",
            ...
            c(names(baseSelect())),
            selected = NULL),
            selectInput("baseTest", "Select test field", c(names(baseSelect())),
            selected = NULL),
            if(input$fields == "onefield") {
                selectInput("baseControl", "Select control field", c(" "),
                        selected = NULL)
                } else {
                    selectInput("baseControl", "Select control field",
                        ...
                        having to re-run"),
                        selectInput("setcolor", "Chart Colors:",
                        c("Default" = "df","Terrain" = "tr", "Rainbow" = "rb",
                        "Heat" = "ht", "Topo" = "tp", "CM" = "cm")))
    ...
    v$graphVC<- renderPlot({
        ...
        if (input$setcolor == "df"){v$colorSet <- c("#180052", "#0c0083",
        ...
        if (input$setcolor == "tr"){v$colorSet <- terrain.colors(n = 20)}
        if (input$setcolor == "rb"){v$colorSet <- rainbow(n = 20)}
        if (input$setcolor == "ht"){v$colorSet <- heat.colors(n = 20)}
        if (input$setcolor == "tp"){v$colorSet <- topo.colors(n = 20)}
        if (input$setcolor == "cm"){v$colorSet <- cm.colors(n = 20)}
        v$graficof <-ggplot(data = v$func_ne_return, aes(x = X, y = Y))+
            ...
            annotate("segment", x = v$matrixSize*1.07, xend = v$matrixSize*1.07,
                ...
                colour = "black", size=0.2, alpha=0.6, arrow=arrow(type =
